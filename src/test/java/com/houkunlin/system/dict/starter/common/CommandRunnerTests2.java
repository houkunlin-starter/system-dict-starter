...
public class CommandRunnerTests2 implements CommandLineRunner {
    ...
    public void run(final String... args) throws Exception {
        ...
        final DictType dictType1 = DictUtil.getDictType(dictType);
        final String raw1 = toJson(dictType1);
        logger.info("初始的完整类型：{}", dictType1);
        logger.info("初始的完整类型 Json：{}", raw1);

        start = System.nanoTime();
        publisher.publishEvent(new RefreshDictValueEvent(DictValue.builder().dictType(dictType).value(2).title("修改后的名称").build()));
        logger.info("修改了一个值 耗时 {} ms", (System.nanoTime() - start) / 1000_000.0);
        final DictType dictType2 = DictUtil.getDictType(dictType);
        ...
        start = System.nanoTime();
        final DictType newDictType = DictType.newBuilder(dictType, dictType1.getTitle(), dictType1.getRemark())
            .add(1, "新的值1")
            .add(2, "新的值2").build();
        publisher.publishEvent(new RefreshDictTypeEvent(newDictType));
        logger.info("更新了一个完整的对象 耗时 {} ms", (System.nanoTime() - start) / 1000_000.0);
        final DictType dictType3 = DictUtil.getDictType(dictType);
        ...
        final DictType dictType4 = DictUtil.getDictType(dictType);
