...
public class DictUtil {
     ...
     */
    public static String TYPE_PREFIX = DictPropertiesStorePrefixKey.TYPE_PREFIX;
    /**
     * 系统字典类型缓存键前缀
     * <p>用于构建系统字典类型在缓存中的键，格式为：dict:t_system:{type}
     */
    public static String TYPE_SYSTEM_PREFIX = DictPropertiesStorePrefixKey.TYPE_SYSTEM_PREFIX;
    /**
     * 字典值缓存键前缀
     * <p>用于构建字典值在缓存中的键，格式为：dict:v:{type}:{value}
     */
    public static String VALUE_PREFIX = DictPropertiesStorePrefixKey.VALUE_PREFIX;
    /**
     * 字典父级值缓存键前缀
     ...
     * @since 1.4.6
     */
    public static String PARENT_PREFIX = DictPropertiesStorePrefixKey.PARENT_PREFIX;
    /**
     * 字典注册器
     * <p>负责管理字典提供者和刷新字典数据
     */
     ...
     * 注意：
     * <ul>
     *     <li>1. 调用此接口后请发起 {@code com.houkunlin.dict.notice.RefreshDictEvent} 事件刷新字典数据，把系统的字典信息写入到新的 {@link DictStore} 存储对象中</li>
