        ...
        if (propertiesCache.isUseCaffeineSpec()) {
            ...
            if (b1 || b2 || b3) {
                ...
                if (b1 && log.isWarnEnabled()) {
                    log.warn("配置参数 system.dict.cache.maximum-size 已经过时，已失效，请使用 system.dict.cache.caffeine.spec");
                }
                if (b2 && log.isWarnEnabled()) {
                    log.warn("配置参数 system.dict.cache.initial-capacity 已经过时，已失效，请使用 system.dict.cache.caffeine.spec");
                }
                if (b3 && log.isWarnEnabled()) {
                    log.warn("配置参数 system.dict.cache.duration 已经过时，已失效，请使用 system.dict.cache.caffeine.spec");
                }
            }
            builder = Caffeine.from(getCaffeineSpec(propertiesCache.getCaffeine()));
        }
